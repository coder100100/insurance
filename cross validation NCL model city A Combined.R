...
N <- 1000     #10                  #number of times to do the cross-validation
years <- c(2002:2011)
...
for(k in 1:length(K)){              # k=1
  ...
  for(i in 1:N){            #i=1                                  
    ...
    #train_data33<-data.frame(wsloss,wspcp,wspcp_lag1, wmpcp,wsncl)
    ...
    y0<-wsncl_train
    x0<- data.frame(wspcp_train,wspcp_lag1_train,wmpcp_train)  
    
    
    #------------------------------------------ GA SVR--------------------------------------------------- 
 
    model_GASVR <- svm(x0,y0, cost =23.09179, gamma =0.1157251 , epsilon =  0.3349581, type = "eps-regression", kernel = "radial")
    
    
    #----------------------glm - Poisson Regression ------------------------------------------------
    
    model_glm<-glm(y0~wspcp_train+wspcp_lag1_train+wmpcp_train, family = poisson)
    
    
    #--------------------- DNN -------------------------------------------------------------------
    
    dataset<-data.frame(wsncl_train,wspcp_train,wspcp_lag1_train,wmpcp_train)
    ...
    train_features <- train_dataset %>% select(-wsncl_train)
    test_features <- test_dataset %>% select(-wsncl_train)
    
    train_labels <- train_dataset %>% select(wsncl_train)
    test_labels <- test_dataset %>% select(wsncl_train)
...
#---------------------------Testing GA-SVR; GLM -------------------------------------------------------
    ...
    x1<- data.frame(wspcp_test,wspcp_lag1_test,wmpcp_test) 
    
    
    #--------------GASVR-------------------------------------------------------
    
    aa1<-predict(model_GASVR, x1, type="response")
    #am1<-mean(na.omit(a1))
    #aa1<-a1 %>% replace_na(am1)  # Replacing NA with the mean
  

    RMSE_GASVR2<- sqrt(mean((wsncl_test- aa1)^2))  #}   # GASVR  # Prediction and RMSE
    RMSE_GASVR<-c(RMSE_GASVR,RMSE_GASVR2)

    
    
    #-----------glm-----------------------------------------------------
    aa3<-predict(model_glm, x1, type="response")
    #am3<-mean(na.omit(a3))
    #aa3<-a3%>% replace_na(am3)  # Replacing NA with the mean
    
    RMSE_glm2<- sqrt(mean((wsncl_test- aa3)^2))            # GLM
    RMSE_glm<-c(RMSE_glm,RMSE_glm2)
    
   
    #--------------------------- DNN ---------------------------------------------

    #y02 <- predict(dnn_model, as.matrix(x1))
    
    aa2<-predict(dnn_model, as.matrix(x1))
    #am2<-mean(na.omit(a2))
    #aa2<-a2 %>% replace_na(am2)  # Replacing NA with the mean
    
    
    RMSE_DNN2<- sqrt(mean((wsncl_test- aa2)^2))  #}   # GASVR  # Prediction and RMSE
...
NC<-(5*sum(wsncl))/10
N_RMSE_glm<-(mean(mat_RMSE[,1])/NC)*100
N_RMSE_DNN<-(mean(mat_RMSE[,2])/NC)*100
N_RMSE_GASVR<-(mean(mat_RMSE[,3])/NC)*100
